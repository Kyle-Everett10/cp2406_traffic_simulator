        ...
        for (int i = 0; i < drawnCars.length; i++) {
            ...
            if (drawnCars[i] != null) {
                if (drawnCars[i].getDirection().equals("north")) {
                    ...
                    g.fillRect(drawnCars[i].getCurrentXFront(), drawnCars[i].getCurrentYBack(), 2, (drawnCars[i].getCurrentYFront() - drawnCars[i].getCurrentYBack()));
                } else if (drawnCars[i].getDirection().equals("east")) {
                    g.fillRect(drawnCars[i].getCurrentXBack(), drawnCars[i].getCurrentYFront(), (drawnCars[i].getCurrentXFront() - drawnCars[i].getCurrentXBack()), 2);
                } else {
                    g.fillRect(drawnCars[i].getCurrentXFront(), drawnCars[i].getCurrentYFront(), (drawnCars[i].getCurrentXBack() - drawnCars[i].getCurrentXFront()), -2);
                }
            }
        }
    }
